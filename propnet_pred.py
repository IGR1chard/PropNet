...
fpath = 'E:/fluentmodel/dao/data_arrays.npz'
...
def model_eva(pred_value, real_value, xloc, zloc, stepnum=2, var_name='fu'):
    ...
    if var_name not in prefix_to_index:
        raise ValueError(f"未知变量名称前缀: {var_name}")
    
    # 获取相应的最大最小值
    min_x = scaler.data_min_[0]  # 如果x是第1列，索引为0
    max_x = scaler.data_max_[0]
    min_z = scaler.data_min_[2]  # 如果z是第3列，索引为2
    ...
    x_value_actual = xloc * (max_x - min_x) + min_x# 反归一化数据集
    z_value_actual = zloc * (max_z - min_z) + min_z# 反归一化数据集
    ...
    ax3.set_title("True proppant volume fraction/%", fontsize=12)
...
pu_pred, pv_pred, pw_pred, fu_pred, fv_pred, fw_pred, p_pred, fvol_pred = predict_time_t(model=model_read, t=104)
model_eva(fvol_pred, fvol_all, xloc=x_all, zloc=z_all, stepnum=104, var_name='fvol')#第1 2 和最后一个参数为你要验证的量
plt.tight_layout()
plt.show()